...
(() => {
  ...
  const btnVerInteira = $("btnVerInteira");
  const lightbox = $("lightbox");
  const lightboxImg = $("lightboxImg");
  const lightboxLegenda = $("lightboxLegenda");
  const btnFecharLightbox = $("btnFecharLightbox");
  let letraAberta = null;
  // guarda a imagem exibida agora (letra ou quadro completo), pra abrir
  // inteira no lightbox
  ...
  function montarFigura(idioma, letra) {
    ...
    figura.classList.remove("tem-imagem");
    imagemAtual = null;
    btnVerInteira.classList.add("hidden");

    const url = Idiomas.caminhoImagem(idioma, letra);
    const img = new Image();
    img.alt = `Como fazer a letra ${letra} em ${Idiomas.nome(idioma)}`;

    img.onload = () => {
      figura.classList.remove("carregando");
      figura.classList.add("tem-imagem");
      figura.innerHTML = "";
      figura.appendChild(img);
      // imagem existe: guarda pra abrir inteira e mostra o botão
      imagemAtual = { url, legenda: `Letra ${letra} — ${Idiomas.nome(idioma)}` };
      btnVerInteira.classList.remove("hidden");
    };
    ...
    img.onerror = () => {
      ...
      figura.innerHTML =
        ...
        `</div>`;
      // sem imagem: sem botão de ver inteira
      btnVerInteira.classList.add("hidden");
    };
  ...
  /** Abre a imagem em tela cheia, no tamanho real, sem corte nenhum. */
  ...
  function abrirQuadroCompleto() {
    ...
    const urlCompleta = Idiomas.caminhoImagemCompleta(idioma);
    const img = new Image();
    ...
    img.onload = () => {
      ...
      figura.innerHTML = "";
      figura.appendChild(img);
      // clicar no pôster abre ele inteiro em tela cheia (útil pra ler as
      // letras pequenas)
      figura.onclick = () => abrirLightbox(urlCompleta,
        `Alfabeto completo de ${Idiomas.nome(idioma)}`);
    };
    img.onerror = () => {
      figura.classList.remove("carregando");
      figura.onclick = null;
  ...
  // abrir a imagem inteira: pelo botão ou clicando na própria figura
  const verInteira = () => {
    if (imagemAtual) abrirLightbox(imagemAtual.url, imagemAtual.legenda);
  };
  btnVerInteira.addEventListener("click", verInteira);
  $("letraFigura").addEventListener("click", () => {
    if (imagemAtual) verInteira();   // só quando há imagem (não no placeholder)
  });

  btnFecharLightbox.addEventListener("click", fecharLightbox);
  lightbox.addEventListener("click", (e) => {
    if (e.target === lightbox) fecharLightbox();   // clicar no fundo fecha
  });
  ...
  document.addEventListener("keydown", (e) => {
    // o lightbox está por cima de tudo: Esc fecha ele primeiro
