...
(() => {
  ...
  // O dataset: lista de amostras { label, landmarks[63], timestamp }
  const dataset = [];
  ...
  /**
   * Normaliza os 21 landmarks em relação ao pulso (landmark 0) e à escala
   * da mão, e devolve um array plano de 63 números. Mesma lógica que o
   * classificador vai usar depois — coletar e classificar precisam casar.
   */
  function normalizeLandmarks(lm) {
    const wrist = lm[0];
    // Escala: distância do pulso até a base do dedo médio (landmark 9)
