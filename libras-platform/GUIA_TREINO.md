...
Para rodar o modelo no navegador (sem servidor), o caminho usual é converter para TensorFlow.js e carregá-lo no lugar das regras do `signs.js`. Em resumo:

1. Treine em Python (Keras/TensorFlow).
2. Converta com o `tensorflowjs_converter`.
3. No `signs.js`, carregue o modelo e troque a função `classify` para usar a predição dele.

Para o protótipo atual, as regras geométricas do `signs.js` já cobrem as letras estáticas mais distintas sem precisar de nada disso — o treino entra quando você quiser ampliar o vocabulário.
...
| Usar | `signs.js` + TensorFlow.js | reconhece em tempo real |
