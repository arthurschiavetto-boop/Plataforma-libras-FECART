...
def main():
    ...
    esperadas = idiomas.letras(args.idioma)
    quantas = (f"{len(esperadas)} letras estáticas"
               if esperadas is not None
               else "alfabeto não declarado, usando as classes do dataset")
    print(f"idioma: {idiomas.nome(args.idioma)} · {quantas} · "
          f"excluídas: {', '.join(sorted(excluidas)) or 'nenhuma'}", flush=True)
